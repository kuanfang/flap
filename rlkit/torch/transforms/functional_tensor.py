...
def get_dimensions(img: Tensor) -> List[int]:
    ...
    channels = 1 if img.dim() == 2 else img.shape[-3]
...
def get_image_num_channels(img: Tensor) -> int:
    ...
    raise TypeError(f"Input ndim should be 2 or more. Got {img.dim()}")
...
def rgb_to_grayscale(img: Tensor, num_output_channels: int = 1) -> Tensor:
    if img.dim() < 3:
        raise TypeError(f"Input image tensor should have at least 3 dimensions, but found {img.dim()}")
...
def _rgb2hsv(img: Tensor) -> Tensor:
    ...
    maxc = torch.max(img, dim=-3)[0]
    minc = torch.min(img, dim=-3)[0]
    ...
    hr = (maxc == r).to(dtype) * (bc - gc)
    hg = ((maxc == g)& (maxc != r)).to(dtype)  * (2.0 + rc - bc)
    hb = ((maxc != g) & (maxc != r)).to(dtype) * (4.0 + gc - rc)
    h = hr + hg + hb
...
def _hsv2rgb(img: Tensor) -> Tensor:
    ...
    mask = i.unsqueeze(dim=-3).to(torch.int64) == torch.arange(6, device=i.device).view(-1, 1, 1)
    ...
    ## Errors on torch 1.0.1.post2 if we don't have this dummy line below
    if img.shape[0] == 1:
        _ = torch.einsum("...ijk, ...xijk -> ...", mask, a4)
    else:
        _ = torch.einsum("...ijk, ...xijk -> ", mask, a4)
    return torch.einsum("...ijk, ...xijk -> ...xjk", mask, a4)
...
def equalize(img: Tensor) -> Tensor:
    ...
    if not (3 <= img.dim() <= 4):
        raise TypeError(f"Input image tensor should have 3 or 4 dimensions, but found {img.dim()}")
    if img.dtype != torch.uint8:
        raise TypeError(f"Only torch.uint8 image tensors are supported, but found {img.dtype}")
